...
def get_code_body(response):
    if "```" in response:
        if "```cpp" in response:
            return response.split("```cpp")[1].split("```")[0]
        else:
...
class PyGenerator:
    def ldb_debug(self, prompt: str, prev_func_impl: str, failed_test: str, entry: str, model: ModelBase, messages: List[Message], dataset_type: str = "", level: str = "block") -> str:
        ...
        failed_test_string = failed_test.split("# Error:")[0]
        real_test_output = failed_test.split("# Error:")[1]
        if model.is_chat:
            if dataset_type in ["TransCoder"]:
                ...
                if len(messages) == 0:
                    messages = [
                        ...
                        Message(
                            role = "user",
                            content = f"Complete the following task in C++. Please respond with code only.\n{prompt}"
                        ),
                        Message(
                            role = "assistant",
                            content = f"{prev_func_impl}"
        ...
        if model.is_chat:
            if dataset_type in ["TransCoder"]:
                ...
                msg = [
                    Message(
                            role = "user",
                            content = "You are an assistant that provides only the canonical solution to C++ code completion prompts. "
                "Your task is to **ONLY** return the correct and complete implementation of the provided function prompt, "
                "starting from the first line of the implementation. "
                "The output must be seamlessly concatenated with the given function definition.\n\n"
                "Format Example:\n"
                "Prompt:\n"
                "/*\n"
                ...
                "}\n\n"
                "Important:\n"
                "- Do not include the function definition line (`int factorial(int n){`)\n"
                "- Do not use any code block formatting, such as ``` or ```cpp. Simply return the raw implementation.",
                        )
                ]
                messages += msg
                print_messages(msg)
